...
@scenario(FEATURE, 'Error handling configured to throw >399, except 404')
def test_error_handling_configuration_raise_for_status_over_399():
    pass


@given('an Api configured to throw on status codes >399, except 404', target_fixture='context')
def context():
    context.api = None
    context.api = Api('http://example.org')
    add_root_to_context(context)
    context.api.error_handling.raise_for_status_codes = '400-403 >404'
    return context
